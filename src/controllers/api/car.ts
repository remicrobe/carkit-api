...
export default class CarController {
    ...
    @AutoDoc({
        ...
        responses: [
            { code: 200, exemple: { id: 'uuid', name: 'My Tesla', make: 'Tesla', model: 'Model S', type: 'elec' } },
    ...
    public async createCar(req: Request, res: Response) {
        try {
            const { type, model, make, name, mileageAtStart, year, imageData } = req.body;
            ...
            if (imageData) {
                car.imageLink = await base64tojpeg(imageData, { height: 800, width: 800 });
            }
    ...
    public async getCar(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const car = await CarRepository.findOneOrFail({ where: { id: Number(id) } });
            return res.send(car);
        } catch (e) {
            return ErrorHandler(e, req, res);
        }
    ...
    @AutoDoc({
        ...
        responses: [
            { code: 200, exemple: { id: 'uuid', name: 'Updated Tesla', make: 'Tesla', model: 'Model S', type: 'elec' } },
            { code: 401, exemple: { status: 401, msg: 'Unauthorized.' } },
            { code: 404, exemple: { status: 404, msg: 'Car not found.' } }
        ]
    })
    public async updateCar(req: Request, res: Response) {
            try {
                const { id } = req.params;
                const { make, model, name, mileageAtStart, year, type, imageData } = req.body;

                const car = await CarRepository.findOneOrFail({ where: { id: Number(id) } });

                if (make) car.fabricant = make;
                ...
                if (imageData) {
                    if (car.imageLink) {
                        await deleteImage(car.imageLink);
                    }
                    car.imageLink = await base64tojpeg(imageData, { height: 800, width: 800 });
                }
